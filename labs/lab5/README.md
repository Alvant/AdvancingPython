...
В данной задаче предлагается простоты ради использовать "упрощённый вариант" стемминга: чтобы упростить слово, надо отрезать от него конец, начиная с первой гласной в последней из подряд идущих групп гласных (😅).
...
В этом случае надо вывести на экран сообщение: `"Не корректное время дня (варианты: day или night)!"` или `"Некорректное число насекомых (нужно целое число)!"`
