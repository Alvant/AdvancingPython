...
В файле [f.txt](./files/D2/f.txt) приведены значения функции $f$ в формате `x -> y`.
А в файле [g.txt](./files/D2/g.txt) приведены значения функции $g$ в формате `y -> z`.
Гарантируется, что область значений $f$ содержится в области определения $g$.

Запишите в файле "result.txt" значения композиции функций $g$ и $f$ в том же формате, причём чтобы строчки файла были ещё упорядочены по аргументу $x$.
...
Это можно сделать с помощью простого подсчёта частот слов (слово, которое чаще встречается, и определяет то, о чём говорится в тексте).
...
Маша понимает, что она учится на Физтехе, а не в лингвистическом институте, но ничего с собой поделать не может: ей нравится изучать языки, и раз есть возможность заниматься и английским, и китайским, то она решила попробовать учить сразу оба языка.
И вроде бы всё ничего, со всем справляется (как Маша всё успевает — загадка).
Однако иногда, в загруженные недели, ей приходится особенно тяжело.
...
Определённого алгоритма у Маши нет, и в этот раз она решила провести такой эксперимент: узнать в начале недели у одногруппников, какой бы они фильм советовали ей посмотреть, выбрать фильм из предложенных, и в конце недели устроить просмотр.
...
Если Маша полностью доверяет мнению человека, то у него стоит рейтинг `1.0`, если же доверия человеку почти нет (либо не особо знакомы, либо есть какие-то причины не доверять), то стоит `0.1` или вообще `0.0`.
...
$$
  s(m) = \sum_{i \in \mathrm{recs}_m} \mathrm{rating}_i
$$

То есть показатель для данного фильма `m` (movie score, или `s`) равен сумме рейтингов доверия одногруппников ($\mathrm{rating}$), которые порекомендовали Маше данный фильм к просмотру ($\mathrm{recs}_m$).
Таким образом, чем больше "надёжных" одногруппников порекоменуют фильм `m`, тем выше получится оценка `s(m)`.

Помогите Маше!
Напишите программу, которая бы по всем упомянутым входным данным посчитала оценку `s(m)`.
Результат запишите в файл "movie_ratings.txt" в формате `movie - s(movie)` и так, чтобы 1) фильмы были расположены по убыванию значения `s(m)` и 2) среди фильмов не было бы тех, которые Маша уже смотрела (такие фильмы просто не надо включать в итоговый файл).
...
В файле [route.txt](./files/D8/route.txt) построчно приведены последовательные места пребывания Алисы во время её путешествия по Стране чудес: `откуда -> куда`.
...
* `add k, v` — добавить в словарь значение `v` по ключу `k` и вывести `v` на экран
* `get k` — достать из словаря значение `v` по ключу `k` и вывести `v` на экран
...
Если в словаре будет два равноудалённых от `k` ключа (один меньше, другой больше) как результат `get k` надо вернуть среднее арифметическое их значений.
...
Во всех задачах можно предлогать и свои модификации — но тогда надо это заранее обсудить с преподавателем.


### Задача Ev1 (Поймай шарик)

<a href="../labs/lab6/README.md#задача-ev1-поймай-шарик">Предыдущее задание</a>.
...
<a href="../labs/lab6/README.md#задача-ev2-тараканьи-бега">Предыдущее задание</a>.
...
<a href="../labs/lab6/README.md#задача-ev3-круги-на-воде">Предыдущее задание</a>.
...
<a href="../labs/lab6/README.md#задача-ev4-листья">Предыдущее задание</a>.
...
<a href="../labs/lab6/README.md#задача-ev5-brick-clicker-или-интересные-картинки">Предыдущее задание</a>.
...
* Добавить блок – "убийцу прогресса": это такой активный блок, который загорается красным (случается редко). Если по такому блоку не успеть кликнуть вовремя или промазать мимо (в зависимости от того, какие модификации были реализованы до этого), то все открытые до этого блоки возвращаются обратно и вся картинка снова закрывается.


### Задача Ev6 (Свободная)

<a href="../labs/lab6/README.md#задача-ev6-свободная">Предыдущее задание</a>.
