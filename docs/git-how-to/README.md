...
В этом случае надо будет проверить, добавлен ли путь до Git в системную переменную `PATH`.
Для этого можно зайти в `Панель управления\Система и безопасность\Система\Дополнительные параметры системы`, кликнуть "Переменные среды", и там будет настройка `PATH`.
